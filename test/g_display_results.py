...
def main():
    ...
    # make_crossover_point_plot()


def make_ci_violations_plot():
    fig, ax = create_violations_axes()
    upper_violations = upper_bound <= 0
    lower_violations = lower_bound >= 0
    all_violations = np.logical_or(upper_violations, lower_violations)
    violations_by_step = np.sum(all_violations, axis=0)
    fraction_violations = violations_by_step / float(N_RUNS)
